# 📦 BƯỚC 1: XÂY DỰNG MÔ HÌNH DỰ ĐOÁN ETA LOGISTICS (ĐẦU VÀO CỦA MLOPS)

Trong thực tế, Data Scientist sẽ đưa cho bạn một file code Python thô để dự đoán thời gian giao hàng (ETA) dựa trên 2 yếu tố chính:
1. **Khoảng cách giao hàng (Distance in km)**
2. **Số điểm dừng giao hàng (Number of stops)**

### 1. Tạo file mã nguồn huấn luyện (`train_eta.py`)
Hãy tạo một thư mục mới tên là `logistics-mlops` và tạo file `train_eta.py` bên trong:
...
### 2. Cần làm gì tiếp theo?

Bạn hãy thực hiện các việc sau trên máy của mình:
1. Tạo thư mục `logistics-mlops`.
2. Tạo file `train_eta.py` và dán đoạn code trên vào.
3. Chạy lệnh cài đặt thư viện cần thiết (nếu chưa có):
   ```bash
...
Khi bạn đã chạy xong bước này, hãy báo lại cho tôi. Lúc đó, bạn chính thức đóng vai trò MLOps Engineer tiếp nhận file code này để **tự động hóa nó qua Airflow/Pipeline hoặc đóng gói Docker**, tôi sẽ hướng dẫn bạn bước tiếp theo ngay lập tức!